...
- **`backend/src/utils/`** — domain logic. Notable: `weather-pipeline.js` / `weather-data.js` (NOAA + Open-Meteo), `avalanche-pipeline.js` / `avalanche-orchestration.js` / `avalanche-detail.js` (zone resolution, bulletin parsing, center-specific fixes), `safety-score.js`, `activity-profiles.js` (per-activity hazard weights, snow-travel avalanche relevance, AI framing; mirrored by `frontend/src/app/activity-profiles.ts` for chapter/check order), `snowpack.js`, `alerts.js`, `precipitation.js`, `fire-risk.js`, `heat-risk.js`, `supplemental-evidence.js`, `ai-client.js`, `http-client.js` (fetch with timeout + circuit breaker).
