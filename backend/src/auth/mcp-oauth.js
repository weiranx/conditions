...
const { describeCallback } = require('./mcp-clients');
...
function createMcpOAuthService({ database, clientId, clientSecret, redirectUris, issuer, frontendOrigin }) {
  ...
  const register = async body => {
    ...
    return {client_id:id,...(secret?{client_secret:secret,client_secret_expires_at:0}:{}),client_id_issued_at:Math.floor(Date.now()/1000),
      client_name:describeCallback(redirects[0]).clientName,redirect_uris:redirects,token_endpoint_auth_method:method,grant_types:['authorization_code','refresh_token'],response_types:['code'],scope:'conditions:read'};
  };
  ...
  const approve = async (request, userId, sessionToken, allow) => database.transaction(async query => {
    ...
    const grant = (await query(`INSERT INTO mcp_oauth_grants (user_id,session_id,client_id,expires_at,redirect_uri)
      VALUES ($1,$2,$3,LEAST($4::timestamptz,NOW()+INTERVAL '30 days'),$5) RETURNING id`, [userId,session.id,item.client_id,session.expires_at,item.redirect_uri])).rows[0];
  ...
  const list = async userId => (await database.query(`SELECT id,client_id,created_at,expires_at,redirect_uri FROM mcp_oauth_grants
    WHERE user_id=$1 AND revoked_at IS NULL AND expires_at>NOW() ORDER BY created_at DESC`, [userId])).rows.map(row=>({...row,...(describeCallback(row.redirect_uri)||{clientName:'MCP client'})}));
