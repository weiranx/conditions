...
AI features require an account and are limited per user by provider-reported token usage. Free accounts default to 250,000 tokens per month and Premium accounts default to 2,000,000. Configure the allowances with `AI_FREE_MONTHLY_TOKEN_LIMIT` and `AI_PREMIUM_MONTHLY_TOKEN_LIMIT`; authenticated account responses include the resolved tier, current meter, and reset date. The legacy `AI_USER_MONTHLY_TOKEN_LIMIT` remains a fallback for the Free allowance.
