...
| GET | `/api/account/objective-watches` | List watches or look up the signed-in account's exact coordinate/date/start/window plan |
| POST | `/api/account/objective-watches` | Create a private watch from a live, saved, or shared report snapshot for a plan date that has not ended; re-saving the same plan replaces its baseline |
| PATCH | `/api/account/objective-watches/:watchId` | Opt a watch into or out of risk-increase email alerts |
| POST | `/api/account/objective-watches/:watchId/review` | Mark the watch's recorded changes reviewed, clearing its needs-attention state until a later change |
...
Objective Watch entitlements are enforced by the backend: Free includes one active watch, manual refresh, in-app updates, and 14 days of check history; Premium includes up to ten active watches, automatic three-hour/hourly checks, verified-email alerts, and 90 days of check history. Every attempted check records whether conditions were unchanged, meaningfully changed, partially available, or unavailable. Each reported change is marked as a risk increase, an improvement, or both; email alerts go out only for risk increases. A source that fails to load counts as unknown rather than as a cleared alert, lifted closure, or lower avalanche danger. Current safety reports and their official source data are not gated by Objective Watch tier checks.
