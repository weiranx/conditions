...
function fixture(t) {
  ...
  const env = {
    ...
    TEST_IMAGE_EXISTS: '1', TEST_ROLLBACK_HEALTHY: '0',
  };
...
if [ "$*" = 'image inspect summitsafe-backend:latest' ]; then
  test "$TEST_IMAGE_EXISTS" = 1
fi
`);
...
test('fast-forwards to the tested SHA and holds the shared lock through the real deploy script', (t) => {
  ...
  assert.match(f.calls(), /image tag summitsafe-backend:latest summitsafe-backend:rollback\ncompose build --pull backend/);
...
test('unhealthy releases fail with diagnostics after bounded readiness attempts', (t) => {
  const f = fixture(t);
  const result = f.run(f.initial, { TEST_HEALTH_EXIT: '28', TEST_IMAGE_EXISTS: '0' });
