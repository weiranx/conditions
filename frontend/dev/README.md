...
Supported workflows include report/search responses, multi-day forecasts, saved reports and local shared links, watch mutations, canned AI brief/chat/snow responses, sample route analysis, and the admin dashboard. The dashboard shows a synthetic week of report and AI traffic, automated health checks, and a handful of extra accounts (`dev/admin-fixtures.mjs`); traffic sits on fixed clock slots, so it stays put between refreshes. Admin feature flags, AI toggles, usage settings, fixture-account actions, the watch scheduler, and cache clearing are local simulations that add audit-trail entries. Operational actions that do not make sense for this fixture server (such as restarting the backend) return an explicit unsupported response; there is no fallback to the live backend. Email only adds an entry to `/api/dev/mock`'s `outbox`; it never sends a message. Background watch scheduling and real billing are not simulated.
