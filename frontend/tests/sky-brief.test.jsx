...
test("feels-like numbers are checked against the matching limit", () => {
  ...
  const heat = numberCard(brief({ ...w, heatRiskLabel: "Elevated" }, hot, "trail-running"), "Warmest feels-like");
...
test("the refreeze number never reads firm without a measured night", () => {
  const withSignals = (signals) => {
    const w = briefWorkspace();
    return numberCard(brief({ ...w, safetyData: { ...w.safetyData, terrainCondition: { code: "snow_ice", label: "Snow", signals } } }, undefined, "snow-climbing"), "Overnight refreeze");
  };
