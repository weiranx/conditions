...
const loadGoogleIdentityApi = () => {
  ...
  googleScriptPromise = new Promise<GoogleIdentityApi>((resolve, reject) => {
    ...
    const rejectLoad = () => reject(new Error('Google sign-in could not load.'));
...
export function GoogleSignInButton({
  ...
  useEffect(() => {
    ...
    void loadGoogleIdentityApi()
      .then((api) => {
        ...
        activeCredentialCallback = (response) => {
          ...
          if (typeof response.credential !== 'string' || !response.credential) {
            errorHandlerRef.current('Google sign-in did not return a credential. Please try again.');
            return;
          }
        ...
        const render = () => {
          if (disposed) return;
          const width = Math.min(400, Math.max(240, Math.floor(buttonElement.getBoundingClientRect().width || 320)));
      ...
      .catch((error) => {
        if (!disposed) {
          errorHandlerRef.current(error instanceof Error ? error.message : 'Google sign-in could not load.');
        }
      });
    ...
    return () => {
      disposed = true;
      observer?.disconnect();
      if (googleCredentialCallback === activeCredentialCallback) googleCredentialCallback = null;
      buttonElement.replaceChildren();
    };
  }, [clientId, nonce]);
  ...
  return (
    <div
      className={`account-google-button${busy ? ' is-busy' : ''}`}
      aria-disabled={busy}
      ref={buttonRef}
    />
  );
