...
import { ArrowDown, ArrowUp, ChevronDown, Maximize2, MessagesSquare, Minimize2, RotateCcw, Sparkles, Square } from "lucide-react";
  ...
  return (
    <section className="field-chat field-panel">
      ...
      <dialog ref={dialog} id={id} className={`field-chat-dialog${fullScreen ? " is-fullscreen" : ""}`} aria-labelledby={`${id}-title`} aria-describedby={`${id}-context`} aria-modal={fullScreen ? true : undefined} onCancel={e => {
        ...
        <div ref={viewport} className="field-chat-messages" tabIndex={0} role="region" aria-label="Conversation messages" onScroll={trackScroll}>
